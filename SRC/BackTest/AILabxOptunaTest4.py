...
使用 Optuna 进行超参数优化(16个参数)，替换原来的多进程网格搜索
...
class AILabxTool:
    def __init__(self, now=None,
                 ...
                 w_bb1=0.2, w_bb2=0.2,
                 w_cc1=1, w_cc2=1,
                 win_trend_score1=25, win_trend_score2=25,
                 win_roc_score1=5, win_roc_score2=10,
                 win_ma_score1=5, win_ma_score2=18,
                 win_ma_score21=5, win_ma_score22=18):
        self.now = now
        ...
        self.w_cc1 = w_cc1
        self.w_cc2 = w_cc2
        self.win_trend_score1 = win_trend_score1
        self.win_trend_score2 = win_trend_score2
        ...
        self.win_ma_score21 = win_ma_score21
        self.win_ma_score22 = win_ma_score22
    ...
    def get_score(self, symbol):
        ...
        trend_score1 = self.trend_score(symbol, "close", self.win_trend_score1)
        trend_score2 = self.trend_score(symbol, "close", self.win_trend_score2)
        ...
        ma_score21 = self.ma(symbol, "volume", self.win_ma_score21)
        ma_score22 = self.ma(symbol, "volume", self.win_ma_score22)
        aa1 = trend_score1
        aa2 = trend_score2
        bb1 = roc_score1
        bb2 = roc_score2
        cc1 = ma_score1 / ma_score2
        cc2 = ma_score21 / ma_score22
        score = (aa1 * self.w_aa1 + aa2 * self.w_aa2
                 + bb1 * self.w_bb1+ bb2 * self.w_bb2
                 + cc1 * self.w_cc1 +  + cc2 * self.w_cc2)
        return score, aa1, bb1, cc1
...
class AILabxStrategy:
    def __init__(self, context, white_list: list = None, max_count: int = 1,
        ...
        self.ailabx = AILabxTool(w_aa1=w_aa1, w_aa2=w_aa2,
                                 w_bb1=w_bb1, w_bb2=w_bb2,
                                 w_cc1=w_cc1, w_cc2=w_cc2,
                                 win_trend_score1=win_trend_score1, win_trend_score2=win_trend_score2,
                                 win_roc_score1=win_roc_score1, win_roc_score2=win_roc_score2,
                                 win_ma_score1=win_ma_score1, win_ma_score2=win_ma_score2,
                                 win_ma_score21=win_ma_score21, win_ma_score22=win_ma_score22)
...
def init(context):
    ...
    context.ai_labx_strategy = AILabxStrategy(context=context, white_list=list(index_list.keys()),
                                              w_aa1=context.paras['w_aa1'],
                                              w_aa2=context.paras['w_aa2'],
                                              w_bb1=context.paras['w_bb1'],
                                              w_bb2=context.paras['w_bb2'],
                                              w_cc1=context.paras['w_cc1'],
                                              w_cc2=context.paras['w_cc2'],
                                              win_trend_score1=context.paras['win_trend_score1'],
                                              win_trend_score2=context.paras['win_trend_score2'],
                                              ...
                                              win_ma_score21=context.paras['win_ma_score21'],
                                              win_ma_score22=context.paras['win_ma_score22'],
...
def on_backtest_finished(context, indicator):
    ...
    data = [
        ...
        context.paras['w_aa1'], context.paras['w_aa2'],
        context.paras['w_bb1'], context.paras['w_bb2'],
        context.paras['w_cc1'], context.paras['w_cc2'],
        context.paras['win_trend_score1'], context.paras['win_trend_score2'],
        context.paras['win_roc_score1'], context.paras['win_roc_score2'],
        context.paras['win_ma_score1'], context.paras['win_ma_score2'],
        context.paras['win_ma_score21'], context.paras['win_ma_score22'],
...
def write_to_file(results, output_file_path=f'./data/AILabxOptunaTest2_info.xlsx'):
    info = pd.DataFrame(results,
                        columns=[
                            ...
                            'w_bb1', 'w_bb2',
                            'w_cc1', 'w_cc2',
                            'win_trend_score1', 'win_trend_score2',
                            'win_roc_score1', 'win_roc_score2',
                            'win_ma_score1', 'win_ma_score2',
                            'win_ma_score21', 'win_ma_score22',
                            'w_dd', 'w_fd'
                        ])
...
def format_trial_result(results, trial):
    results.append([
        ...
        trial.params.get('w_aa1', 0),
        trial.params.get('w_aa2', 0),
        trial.params.get('w_bb1', 0),
        trial.params.get('w_bb2', 0),
        1,  # w_cc 固定为1
        trial.params.get('w_cc2', 0),
        trial.params.get('win_trend_score1', 0),
        trial.params.get('win_trend_score2', 0),
        ...
        trial.params.get('win_ma_score21', 0),
        trial.params.get('win_ma_score22', 0),
...
def objective(trial):
    ...
    """
    # 定义搜索空间
    w_aa1 = trial.suggest_float('w_aa1', -0.20, 1.10)   # default: 0.1
    w_aa2 = trial.suggest_float('w_aa2', -0.20, 1.10)   # default: 0.1

    w_bb1 = trial.suggest_float('w_bb1', 0.10, 3.50)   # default: 1.6
    w_bb2 = trial.suggest_float('w_bb2', 0.10, 3.50)   # default: 1.6

    # w_cc1 = trial.suggest_float('w_cc1', 0.10, 3.50)   # default: 1.6
    w_cc2 = trial.suggest_float('w_cc2', 0.10, 3.50)   # default: 1.6

    win_trend_score1 = trial.suggest_int('win_trend_score1', 15, 40) # default: 25
    win_trend_score2 = trial.suggest_int('win_trend_score2', 15, 40) # default: 25
    ...
    win_ma_score21 = trial.suggest_int('win_ma_score21', 2, 8) # default: 5
    win_ma_score22 = trial.suggest_int('win_ma_score22', 8, 35) # default: 18
    ...
    paras = {
        ...
        "w_cc1": 1,
        "w_cc2": w_cc2,
        "win_trend_score1": win_trend_score1,
        "win_trend_score2": win_trend_score2,
        "win_roc_score1": win_roc_score1,
        "win_roc_score2": win_roc_score2,
        "win_ma_score1": win_ma_score1,
        "win_ma_score2": win_ma_score2,
        "win_ma_score21": win_ma_score21,
        "win_ma_score22": win_ma_score22,
        "w_dd": w_dd,
        "w_fd": w_fd
    }

    print(f"试验 {trial.number}: 测试参数 w_aa1={w_aa1:.2f}, w_aa2={w_aa2:.2f}, "
          f"w_bb1={w_bb1:.2f}, w_bb2={w_bb2:.2f}, "
          f"w_cc1=1, w_cc2={w_cc2:.2f},, "
          f"win_trend_score1={win_trend_score1}, win_trend_score2={win_trend_score2}, "
          f"win_roc_score1={win_roc_score1}, win_roc_score2={win_roc_score2}, "
          f"win_ma_score1={win_ma_score1}, win_ma_score2={win_ma_score2}， "
          f"win_ma_score21={win_ma_score21}, win_ma_score22={win_ma_score22}， "
...
if __name__ == '__main__':
    # 使用数据库存储确保进程安全
    storage_url = "sqlite:///AILabxOptunaStudy3_002.db"
