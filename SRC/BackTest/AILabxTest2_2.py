...
- operate as follow: (bug sigal) + (hold list) - (sell sigal), and then sort
...
class AILabxStrategy:
    def __init__(self, context, white_list: list = None, max_count: int = 1, w_aa=0.4, w_bb=0.2, w_cc=1, w_dd=0.17):
        self.now = None
    ...
    def filter_should_sell(self, in_list: list=None):
        if in_list is None:
            in_list = []
        symbol_list = [item for item in in_list if not self.should_sell(item)]
        return symbol_list
    ...
    def execute(self, now):
        ...
        ret_list = self.filter_should_sell(ret_list)
