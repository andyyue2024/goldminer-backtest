...
def objective(trial):
    ...
    """
    # 定义搜索空间
    w_aa = trial.suggest_float('w_aa', 0.35, 0.60, step=0.05)
    w_bb = trial.suggest_float('w_bb', 0.10, 2.00, step=0.05)
    w_dd = trial.suggest_float('w_dd', 0.15, 0.20, step=0.01)
    # w_fd = trial.suggest_categorical('w_fd', [18])  # 固定值，但保持参数形式
    w_fd = trial.suggest_int('w_fd', 18, 20)  # 固定值，但保持参数形式
    ...
    print(f"试验 {trial.number}: 测试参数 w_aa={w_aa:.2f}, w_bb={w_bb:.2f}, w_dd={w_dd:.2f}, w_fd={w_fd}")
...
if __name__ == '__main__':
    ...
    study = optuna.create_study(
        direction='maximize',
        sampler=TPESampler(seed=42)  # 使用 TPE 采样器，可替换为 RandomSampler 或 GridSampler
    )
    ...
    best_params_df.to_excel('./data/AILabxTest2_3_optuna_best_params.xlsx', index=False)
    ...
    try:
        ...
        print("要使用可视化功能，请安装: pip install plotly")
